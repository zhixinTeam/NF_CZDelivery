...
uses
  ...
  UFormpoundAdjust,
  //磅单勘误
  UFormGetStock,UFramePoundMtAuto,UFramePoundMtAutoItem,UFramePoundMtQuery,
  //码头抓斗秤
...
begin
  ...
  nStr := Format(nStr, [sTable_SysDict, sFlag_BatcodeDefaultValidDays]);
  gSysParam.FBatcodeDefaultValidDays := '3';
  with FDM.QueryTemp(nStr) do
  if RecordCount > 0 then
  begin
    gSysParam.FBatcodeDefaultValidDays := FieldByName('d_value').AsString;
  end;

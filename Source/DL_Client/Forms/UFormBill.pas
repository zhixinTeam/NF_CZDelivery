...
var nPrint: Boolean;
  nShowReportStockBill,nAskPrintStockBill:Boolean;
  nStr :string;
begin
  nShowReportStockBill := True;
  nAskPrintStockBill := True;
  ...
  nStr := Format(nStr,[sTable_SysDict,sFlag_ShowReportStockBill]);
  with fdm.QuerySQL(nStr) do
  begin
    if RecordCount>0 then
    begin
      nShowReportStockBill := FieldByName('d_memo').AsString=sFlag_Yes;
    end;
  end;

  nStr := 'select * from %s where d_name=''%s''';
  nStr := Format(nStr,[sTable_SysDict,sFlag_AskPrintStockBill]);
  with fdm.QuerySQL(nStr) do
  begin
    if RecordCount>0 then
    begin
      nAskPrintStockBill := FieldByName('d_memo').AsString=sFlag_Yes;
    end;
  end;  

  if nPrint then
    PrintBillReport(FBills, nAskPrintStockBill,nShowReportStockBill);
  //print report

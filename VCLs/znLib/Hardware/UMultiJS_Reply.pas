...
uses
  ...
  USysLoger, IdGlobal, IdTCPConnection, IdTCPClient, UMemDataPool,U02NReader;
...
type
  ...
  //显示屏事件
  TMultiJSLEDProc = procedure (const nTruck,nTunnel:string;const nDaiNum: Integer;const nStockname:string='') of object;

  //
  TMutiJSMutexEvent = function (const nCard: string; nTunnel: string;const nHost:PReaderHost=nil;const nAutoSwitch:Boolean=False):Boolean;
  ...
  private
    ...
    //事件相关
    FLedCounterProc:TMultiJSLEDProc;
    FMutexEvent:TMutiJSMutexEvent;
  protected
    ...
    //新建数据
    function AutoStartNextJS(nTunnel: PMultiJSTunnel):Boolean;
  public
    constructor Create;
    destructor Destroy; override;
    //创建释放
    ...
    function AddJS(const nTunnel,nTruck,nBill:string; nDaiNum: Integer;
      const nDelJS: Boolean = False;const nStockName:string='';const nLedShow:Boolean=True): Boolean;
    //添加计数
    ...
    property LedCounterProc:TMultiJSLEDProc read FLedCounterProc write FLedCounterProc;
    property MutexEvent:TMutiJSMutexEvent read FMutexEvent write FMutexEvent;
    //属性相关
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FMutexId := '';
          nTN := nNode.FindNode('MutexId');
          if Assigned(nTN) then
          begin
            FMutexId := nTN.ValueAsString;
          end;
        end;
      end;
    end;
...
function TMultiJSManager.AddJS(const nTunnel, nTruck, nBill: string;
  nDaiNum: Integer; const nDelJS: Boolean;const nStockName:string;const nLedShow:Boolean): Boolean;
var nStr: string;
    nPH: PMultiJSHost;
    nPT: PMultiJSTunnel;
...
begin
  ...
  WriteLog('DelJS:::nTunnel='+nTunnel+',nPT.FDaiNum='+IntToStr(nPT.FDaiNum)+',nPT.FHasDone='+IntToStr(nPT.FHasDone));  
  {$ENDIF}
...
function TMultiJSManager.AutoStartNextJS(nTunnel: PMultiJSTunnel): Boolean;
var
  nHost:PReaderHost;
begin
  Result := False;
  ...
  begin
    nHost := g02NReader.GetReaderHost(nTunnel.FMutexID);
    if (nTunnel.FMutexID<>'') and (nHost.FNextCard<>'') then
    begin
      if Assigned(FMutexEvent) then
      begin
        FEnableQuery := False;
        FMutexEvent(nHost.FNextCard,nTunnel.FMutexID,nHost,True);
        nHost.FNextCard := '';
        FEnableQuery := True;
        Result := True;
      end;
